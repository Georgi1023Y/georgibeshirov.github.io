...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full max-w-full border-b border-slate-200/80 bg-white/80 pt-[env(safe-area-inset-top,0px)] shadow-sm backdrop-blur-xl backdrop-saturate-150 dark:border-slate-700/60 dark:bg-slate-950/75">
      <nav className="mx-auto flex h-16 min-w-0 max-w-content items-center justify-between gap-2 px-4 sm:gap-3 sm:px-gutter-sm lg:px-gutter-lg">
        ...
        <div className="flex min-w-0 flex-shrink-0 items-center gap-1 sm:gap-2">
          ...
          <button
            className="inline-flex h-11 min-h-[44px] min-w-[44px] items-center justify-center rounded-ds-md text-slate-700 dark:text-slate-200 md:hidden"
            onClick={() => setOpen((o) => !o)}
          ...
          <div className="hidden items-center gap-0.5 md:flex lg:gap-2">
            {navLinks.map((link) => {
              ...
              return (
                <a
                  ...
                  className="inline-flex min-h-[44px] items-center gap-1.5 rounded-ds-full px-3 py-2 text-xs font-medium text-slate-600 transition hover:bg-slate-100/90 hover:text-indigo-600 dark:text-slate-300 dark:hover:bg-slate-800/80 dark:hover:text-indigo-400 lg:px-3.5 lg:text-sm"
                >
